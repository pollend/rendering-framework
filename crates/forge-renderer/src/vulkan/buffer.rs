use crate::{
    check_vk_result,
    types::{DescriptorType, ResourceMemoryUsage},
    vulkan::VulkanRenderer,
    Buffer, VulkanAPI,
};
use gpu_allocator::vulkan::{Allocation, Allocator};
use log::error;
use std::{borrow::BorrowMut, ffi::c_void, ptr, sync::Arc};
            ...
            Some(renderer) => {
                unsafe {
                    if self.vk_storage_texel_view != ash::vk::BufferView::null() {
                        renderer
                            .device
                            .destroy_buffer_view(self.vk_storage_texel_view, None);
                        self.vk_storage_texel_view = ash::vk::BufferView::null();
                    }

                    if self.vk_uniform_texel_view != ash::vk::BufferView::null() {
                        renderer
                            .device
                            .destroy_buffer_view(self.vk_uniform_texel_view, None);
                        self.vk_storage_texel_view = ash::vk::BufferView::null();
                    }
...
impl Buffer<VulkanAPI> for VulkanBuffer {
    unsafe fn map_buffer(&mut self, offset: u32, size: u32) {
        assert!(
            self.memory_usage == ResourceMemoryUsage::GpuOnly,
            "Trying to map non-cpu accessible resource"
        );
        match Arc::get_mut(&mut self.renderer) {
            None => {
